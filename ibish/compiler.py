...
NA_VALUES = ("NA", "NULL", "nan", "NaN", "NAN", r"\0")
NULL_PAT = "|".join(NA_VALUES)
...
def lit(op, *, value, dtype, **_):
    if dtype.is_string():
        # extremely production ready
        return f'"{value}"'
    elif dtype.is_boolean():
        # booleans are integers in awk
...
def count_star(op, *, where, **_):
    if where is None:
        return "++"
    return f" += mkbool({where})"


def not_null(arg):
    return f'!match({arg}, "{NULL_PAT}")'


@translate.register(ops.Count)
def count(op, *, arg, where):
    arg_is_not_null = not_null(arg)
    if where is None:
        return f" += mkbool({arg_is_not_null})"
    return f" += mkbool({where} && {arg_is_not_null})"


@translate.register(ops.Sum)
def total(op, *, arg, where):
    agg = f" += {arg} * {not_null(arg)}"
...
def mean(op, *, arg, where):
    if where is not None:
        raise NotImplementedError("mean with filter not implemented")
    return f" += {arg}"
...
@singledispatch
def postproc(op, **_) -> list[str]:
    return ""


@postproc.register(ops.Mean)
def mean_postproc(op, **_):
    # can't remove spaces because of awk reasons
    return "/NR"
